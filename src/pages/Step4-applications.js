...
const Step4Applications = () => {
  ...
  const handleSuccessGoogleLogin = async (response) => {
    ...
    if (response && response.access_token) {
      ...
      if (userInfo) {
        ...
        sendGoogleUserInfoToBackend(userInfo);
      }
    } else {
  ...
  const sendGoogleUserInfoToBackend = async (userInfo) => {
    console.log("Sending user info to backend", userInfo);
    await fetch(endpoints.login.basic.setGoogleUserData,
      {
        method: "POST",
        headers: {
    ...
    .catch((error) => {
      alert("Error in sendUserInfoToBackend", error);
    });
  };
...
{/* 
        <div className="rounded-full p-2 border shadow-[0_0_10px_rgba(255,255,255,07)]">
          <svg
            className="w-6 h-6"
            viewBox="0 0 24 24"
            fill="none"
